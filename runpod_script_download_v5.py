...
DOWNLOAD_DIR = "/Users/a12/Downloads/RunPod_Script_V5"
...
def monitor_and_download(target_count=8):
    print(f"🚀 [RunPod] V5 자동 다운로드 기동 (경로: {DOWNLOAD_DIR})")
    ...
    while True:
            ...
            for n_id in outputs:
                if 'images' in outputs[n_id]:
                    for img in outputs[n_id]['images']:
                        fname = img['filename']
                        if fname.startswith("Script_V5_Scene_") and fname not in downloaded_files:
        ...
        if not found_new: time.sleep(5)
            ...
            if len(downloaded_files) >= target_count:
                print(f"🎉 V5 장면 {target_count}장 수거 완료!")
                break
