...
DOWNLOAD_DIR = "/Users/a12/Downloads/RunPod_Script_V4"
...
def monitor_and_download(target_count=8):
    print(f"🚀 [RunPod] V4 자동 다운로드 시스템 기동 (대상: {DOWNLOAD_DIR})")
    print(f"🔎 'Script_V4_Scene_' 접두사가 붙은 파일을 수거합니다...")
    ...
    while True:
            ...
            for node_id in outputs:
                if 'images' in outputs[node_id]:
                    for img in outputs[node_id]['images']:
                        ...
                        if filename.startswith("Script_V4_Scene_") and filename not in downloaded_files:
        ...
        if not found_new:
            ...
            if len(downloaded_files) >= target_count:
                print(f"🎉 모든 {target_count}개 V4 장면 수거가 끝났습니다!")
                break
