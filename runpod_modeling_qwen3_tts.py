...
class Qwen3TTSTalkerRotaryEmbedding(nn.Module):
    def __init__(self, config: Qwen3TTSTalkerConfig, device=None):
        ...
        self.config = config
                # Perfect RoPE patch for transformers 4.45+
        if not hasattr(self, 'inv_freq'):
            dim = self.config.hidden_size // getattr(self.config, 'num_attention_heads', 1)
            base = getattr(self.config, 'rope_theta', 10000.0)
            inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2).float() / dim))
            self.register_buffer('inv_freq', inv_freq, persistent=False)
        self.attention_scaling = 1.0
        def dummy_rope_init(config, device=None): return self.inv_freq, 1.0
        self.rope_init_fn = dummy_rope_init

        # inv_freq handled by patch
        self.register_buffer("inv_freq", inv_freq, persistent=False)
