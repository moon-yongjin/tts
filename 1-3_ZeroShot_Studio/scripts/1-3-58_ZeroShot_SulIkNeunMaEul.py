...
# 2. 데이터 설정 (술익는마을 레퍼런스)
REF_AUDIO = "/Users/a12/projects/tts/voices/Reference_Videos/SulIkNeunMaEul_Ref.mp4"
REF_TEXT = "술익는 마을 마다 타는 저녁 놀, 구름에 달 가듯이 가는 나그네."
...
def normalize_text(text):
    ...
    def int_to_sino(num):
        ...
        for i, d_char in enumerate(num_str):
            d = int(d_char)
            pos = length - 1 - i
            unit_pos = pos % 4
            big_unit_pos = pos // 4
            if d > 0:
                if d == 1 and unit_pos > 1: # 100(백), 1000(천) 전용
                    res += units[unit_pos]
                elif d == 1 and unit_pos == 1: # 10(십) 전용 (일십 방지)
                    res += "십"
                else:
                    res += digits[d] + units[unit_pos]
            if unit_pos == 0 and big_unit_pos > 0:
                # 간단화: 큰 단위 (만 등) 붙이기
                res += big_units[big_unit_pos]
        return res

    # 🚀 숫자(\d+) 전체를 한글(Sino-Korean) 발음으로 일괄 치환
    text = re.sub(r'(\d+)', lambda m: int_to_sino(int(m.group(1))), text)
...
def main():
    print("==========================================")
    print("🎙️ Qwen3-TTS [MLX] Zero-Shot Voice Clone (술익는마을)")
    print("==========================================")
