...
# 2. 데이터 설정 (YouTube 레퍼런스)
REF_AUDIO = "/Users/a12/projects/tts/YouTube_Ref.wav"
# 시청각 자료를 토대로 추정된 레퍼런스 텍스트 (사용자 확인 필요할 수 있음)
...
def normalize_text(text):
    ...
    # 마침표를 쉼표로 변환 (KangJiYoung2에서 적용한 해결법)
    text = text.replace('. ', '.').replace('.', '.. ')
...
def main():
    print("==========================================")
    print("🎙️ Qwen3-TTS [MLX] Zero-Shot Voice Clone (YouTube Ref)")
    print("==========================================")
    ...
    if len(combined_audio) > 0:
        ...
        output_path = OUTPUT_DIR / f"YouTube_Ref_Clone_{timestamp}.wav"
