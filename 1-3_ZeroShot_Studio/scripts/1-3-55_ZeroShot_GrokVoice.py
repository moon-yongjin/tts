...
sys.path.append(str(PROJECT_ROOT)) # 모듈 로딩을 위해 추가
...
# 2. 데이터 설정 (Grok 비디오 기반 보컬 레퍼런스)
REF_AUDIO = "/Users/a12/Downloads/extracted_assets/grok-video-fa9ecb66-9c6c-4482-80c4-8c8e115431cf/grok-video-fa9ecb66-9c6c-4482-80c4-8c8e115431cf_vocals.wav"
# 이 보컬의 실제 텍스트가 필요하지만, Zero-shot 특성상 앞부분 80자 정도만 대략적으로 활용합니다.
REF_TEXT = "이건 샘플 오디오 텍스트입니다. 목소리 톤과 억양을 참고하기 위한 레퍼런스입니다."
...
def main():
    print("==========================================")
    print("🎙️ Qwen3-TTS [MLX] Zero-Shot Voice Clone (Grok Vocals)")
    print("==========================================")
    ...
    print(f"\n🚀 모델 로딩 중: {MODEL_PATH.name}...")
    model = load(str(MODEL_PATH))

    print(f"🎧 레퍼런스 오디오 로딩: {os.path.basename(REF_AUDIO)}")
    if not os.path.exists(REF_AUDIO):
        print(f"⚠️ 레퍼런스 오디오가 없습니다: {REF_AUDIO}")
        return

    ref_wav, sr = librosa.load(REF_AUDIO, sr=24000, duration=10.0) # 10초 정도 참고
    ...
    if len(combined_audio) > 0:
        ...
        output_name = f"GrokVoice_ZeroShot_{timestamp}"
