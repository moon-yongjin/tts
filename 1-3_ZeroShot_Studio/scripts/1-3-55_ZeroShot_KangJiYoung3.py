...
# 2. 데이터 설정 (강지영3 레퍼런스)
REF_AUDIO = "/Users/a12/projects/tts/voices/Reference_Videos/KangJiYoung3_Ref.mp4"
REF_TEXT = "강지영 아나운서 스타일의 목소리 톤을 참고하기 위한 레퍼런스 데이터입니다."
...
def main():
    print("==========================================")
    print("🎙️ Qwen3-TTS [MLX] Zero-Shot Voice Clone (강지영3)")
    print("==========================================")
    ...
    for i, chunk in enumerate(chunks):
        # [수정] 말끝 짤림 방지를 위해 문장 끝에 마침표가 없으면 강제로 추가
        processing_chunk = chunk
        if not processing_chunk.endswith(('.', '!', '?', ',')):
            processing_chunk += "."
            
        print(f"🎙️  [{i+1}/{len(chunks)}] 생성 중: {processing_chunk[:40]}...")
        
        try:
            results = model.generate(
                text=processing_chunk,
