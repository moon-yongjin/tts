...
let isRunning = false;
let isPaused = false;
let _pinnedTabId = null; // 한 번 찾은 탭 ID를 고정 저장
let autoDownloadTimer = null;
let pendingDownloadOrder = null;
...
async function getFlowTab() {
  // 1. 고정된 탭 ID가 있으면 그걸 우선 사용
  if (_pinnedTabId !== null) {
    try {
      const pinned = await chrome.tabs.get(_pinnedTabId);
      if (pinned && pinned.url && pinned.url.includes('labs.google')) return pinned;
    } catch (e) {
      _pinnedTabId = null; // 탭이 닫혔으면 초기화
    }
  }
  // 2. 모든 창에서 labs.google 탭 탐색 (active 여부 무관)
  const flowTabs = await chrome.tabs.query({ url: 'https://labs.google/*' });
  if (flowTabs.length > 0) {
    _pinnedTabId = flowTabs[0].id;
    return flowTabs[0];
  }
  // 3. URL 패턴이 달라도 title/url에 labs.google 포함된 탭 탐색
  const allTabs = await chrome.tabs.query({});
  const found = allTabs.find(t => t.url && t.url.includes('labs.google'));
  if (found) {
    _pinnedTabId = found.id;
    return found;
  }
  return null;
}
...
async function sendToContent(action, data = {}) {
  ...
  if (!tab) throw new Error('Flow 탭을 찾을 수 없습니다 (labs.google 탭을 열어주세요)');
...
$('btnStart').addEventListener('click', async () => {
  ...
  const settings = {
    ...
    genTimeout: 120,
    downloadWait: parseInt($('downloadWait').value) || 120,
...
// ========== 다운로드 관련 ==========
function startAutoDownloadCountdown(seconds) {
  let remaining = seconds;
  log(`⏳ ${remaining}초 후 자동 다운로드 시작 (생성 완료 대기 중)...`);

  autoDownloadTimer = setInterval(() => {
    remaining--;
    setStatus(`⏳ 다운로드 대기: ${remaining}초`, '#e2b93b');
    if (remaining <= 0) {
      clearInterval(autoDownloadTimer);
      autoDownloadTimer = null;
      triggerAutoDownload();
...
async function triggerAutoDownload() {
  log('📥 1분 경과 — 자동 다운로드 시작!');
  setStatus('📥 다운로드 중...', '#00d2ff');
  try {
    await sendToContent('downloadAll', { order: pendingDownloadOrder });
  } catch(e) {
    log(`❌ 다운로드 전송 실패: ${e.message}`);
    setStatus('❌ 다운로드 실패', '#e94560');
  }
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === 'log') log(msg.text);
    ...
    resetUI();
    setStatus('✅ Push 완료 — 다운로드 대기 중', '#e2b93b');
    log('✅ 모든 프롬프트 Push 완료');
    pendingDownloadOrder = msg.submittedOrder || [];
    const waitSec = parseInt($('downloadWait').value) || 120;
    startAutoDownloadCountdown(waitSec);
  }
  else if (msg.type === 'downloadDone') {
    setStatus('✅ 다운로드 완료!', '#0f0');
    log('✅ 전체 다운로드 완료!');
    pendingDownloadOrder = null;
  }
  else if (msg.type === 'progress') {
    setStatus(`🚀 ${msg.current}/${msg.total} 처리 중`, '#0f0');
    // 자동화가 실행 중임을 UI에 반영 (패널 재오픈 시 복구)
    if (!isRunning) {
      isRunning = true;
      isPaused = false;
      $('btnPause').textContent = '⏸ 일시정지';
...
(async () => {
  try {
    ...
    if (res && res.automationRunning) {
      isRunning = true;
      isPaused = res.state === 'paused';
      setStatus(isPaused ? '⏸ 일시정지' : '🚀 실행 중 (복구됨)', isPaused ? '#e2b93b' : '#0f0');
      $('btnPause').textContent = isPaused ? '▶ 재개' : '⏸ 일시정지';
      log(`🔄 패널 재오픈 — 자동화 계속 진행 중 (상태: ${res.state})`);
    }
  } catch (e) {
    // 아직 content script 없으면 정상 (무시)
  }
})();
