...
workflow_template = {
  ...
  "9": {
    "inputs": { "filename_prefix": "RunPod_MILSexy", "images": ["17", 0] },
    "class_type": "SaveImage"
  }
}

# 캐릭터 앵커 프롬프트 (시어머니 - 수수한 얼굴, 야한 옷, 큰 가슴)
# "원래보다 얼굴은 좀더 수수하게", "할머니지만 옷은 야하게 가슴은 풍만하게"
BASE_CHARACTER_PROMPT = "A beautiful Korean woman in her late 50s to early 60s, elegant and natural grandmotherly image, salt and pepper or dark elegant hair in a neat style, very plain and modest natural face, humble and simple appearance, extremely large breasts, very voluptuous and curvy full figure, not skinny, healthy glamorous body, wearing very sexy and provocative clothing, low-cut revealing outfit, masterpiece, high quality, photorealistic, 8k, sharp focus"
...
def run_runpod_character_batch(count=10):
    print(f"🚀 [RunPod] 시어머니 캐릭터(수수한 페이스/섹시 바디) 데이터셋 생성을 시작합니다 (총 {count}장)")
    ...
    for i in range(count):
        ...
        wf["16"]["inputs"]["seed"] = int(time.time() * 1000) + 15000 + i
        wf["9"]["inputs"]["filename_prefix"] = f"RunPod_MILSexy_{idx:02d}"
