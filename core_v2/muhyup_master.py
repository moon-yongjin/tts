...
def get_latest_folder():
    ...
    for d in os.listdir(DOWNLOADS_DIR):
        # Normalize NFD (Mac) to NFC for matching
        norm_d = unicodedata.normalize('NFC', d)
        if os.path.isdir(os.path.join(DOWNLOADS_DIR, d)) and norm_d.startswith("무협"):
            folders.append(os.path.join(DOWNLOADS_DIR, d))
...
def run_integrated_render():
    ...
    if not target_dir:
        print("❌ '무협_'으로 시작하는 폴더를 찾을 수 없습니다.")
        return
    ...
    # [NEW] 마스터 오디오 찾기 (WAV/MP3 지원)
    def get_master_audio(target_dir):
        # 1. 폴더 내부 검색
        for ext in [".wav", ".mp3"]:
             for name in [f"audio{ext}", f"final{ext}", f"master{ext}"]:
                 path = os.path.join(target_dir, name)
                 if os.path.exists(path): return path
        
        # 2. 외부(Downloads) 검색 - 최신 파일
        candidates = []
        for f in os.listdir(DOWNLOADS_DIR):
            if f.lower().endswith((".wav", ".mp3")) and ("태양" in f or "무협" in f or "Audio_Full" in f):
...
"""
    with open(ass_path, "w", encoding="utf-8") as f:
        ...
        for ev in srt_events:
            ...
            if content:
                # \fax-0.1 태그로 미세 기울기 적용 (숫자가 작을수록 덜 기울어짐)
    ...
    # [FIX] Using relative paths for FFmpeg on Mac to avoid filter parsing errors
    # Since we use cwd=target_dir, 'final.ass' is in the current directory.
    # relative_fonts_dir = os.path.relpath(BASE_DIR, target_dir)

    # [FIX] FFmpeg command preparation
    # Move to target_dir for relative path safety
    ...
    if master_audio:
        ...
        # Use filter_complex to map video and external audio
        v_filter = f"[0:v]subtitles=filename='final.ass'[vout]"
        cmd.extend(["-filter_complex", v_filter, "-map", "[vout]", "-map", "1:a"])
        print(f"🎙️ 오디오 결합 중: {os.path.basename(master_audio)}")
    else:
        # No external audio, use audio from video clips
        cmd.extend(["-vf", "subtitles=filename='final.ass'"])
        print("⚠️ 외부 오디오를 찾지 못해 영상 소리만 사용합니다.")
    ...
    print(f"🎬 렌더링 시작 (Output: {os.path.basename(output_file)})")
    try:
