...
if os.path.exists(CONFIG_PATH):
    try:
        with open(CONFIG_PATH, "r", encoding="utf-8") as f:
            ...
            print(f"✅ Config 로드 성공: Gemini API Key 확인됨")
    except: pass

# 서비스 계정 파일이 있으면 우선 사용 (개발자용)
...
if os.path.exists(CREDENTIALS_PATH):
    ...
    # 윈도우 배포용: API Key 직접 사용
    client = genai.Client(api_key=GOOGLE_API_KEY)
else:
    print("❌ API Key 또는 서비스 계정을 찾을 수 없습니다.")
    sys.exit(1)

import argparse

# Argument Parsing
parser = argparse.ArgumentParser(description="AI Image Director")
...
if args.output_dir:
    ...
    if not os.path.exists(DOWNLOAD_DIR):
        os.makedirs(DOWNLOAD_DIR)
        print(f"📂 새 폴더 생성됨 (바탕화면): {DOWNLOAD_DIR}")
...
# 스타일 우선순위: Argument > Config > Default
...
def get_visual_bible_proposal(script_text):
    ...
    3. [Context] The story is set in a Korean martial arts (Muhyup) world. Characters are likely Korean.
    4. Provide descriptions in both Korean (for user review) and English (for image generation).
    ...
    for attempt in range(3):
        try:
            print(f"🤖 Visual Bible 생성 시도 ({attempt+1}/3)...")
    ...
    # [Fallback] 모든 시도 실패 시 기본값 생성
    print("❌ API 호출 3회 실패. 기본 설정으로 진행합니다.")
    default_bible = {
        "approved": True, # 기본값은 자동 승인 처리
        "character_ko": "무협풍의 주인공",
        "character_en": "Asian martial arts warrior, traditional robes, determined expression",
        "background_ko": "고전적인 무협 배경",
        "background_en": "Historical asian landscape, atmospheric, cinematic lighting",
        "art_style_override_ko": "수묵화 스타일",
        "art_style_override_en": "Traditional Korean ink wash painting style",
        "visual_variety_ko": "기본 연출",
        "visual_variety_en": "Standard cinematic composition",
        "target_interval": 6.0,
        "clip_duration": 6.0
    }
    try:
        with open(SETTINGS_PATH, "w", encoding="utf-8") as f:
            json.dump(default_bible, f, ensure_ascii=False, indent=2)
        print(f"✅ [안전 모드] 기본 설정 파일이 생성되었습니다: {SETTINGS_PATH}")
        return True
    except Exception as e:
        print(f"❌ 기본 파일 생성조차 실패: {e}")
        return False
...
def get_character_profile(script_text, script_name="대본.txt"):
    ...
    4. [Context] The story is set in a Korean martial arts (Muhyup) world. The protagonist is Korean.
    5. Create a "Consistency Prompt" for image generation.
    ...
    try:
        ...
        try:
            # [추가] 대본 식별을 위한 시작 부분 저장
            profile['script_start'] = script_text[:50].strip()
...
def run_pipeline(full_text, script_name="대본.txt"):
    ...
    if not os.path.exists(SETTINGS_PATH):
        ...
        if get_visual_bible_proposal(full_text):
            ...
            if args.auto_approve:
                print("⏩ [Auto-Approve] 설정 자동 승인 및 진행")
                try:
                    with open(SETTINGS_PATH, "r", encoding="utf-8") as f:
                        bible = json.load(f)
                    bible["approved"] = True
                    # 기본 스타일 설정 (자동 모드일 때)
                    bible["visual_variety_ko"] = VISUAL_DIRECTIONS["1"]["ko"]
                    bible["visual_variety_en"] = VISUAL_DIRECTIONS["1"]["en"]
                    with open(SETTINGS_PATH, "w", encoding="utf-8") as f:
                        json.dump(bible, f, ensure_ascii=False, indent=2)
                except Exception as e:
                    print(f"⚠️ 자동 승인 처리 중 오류: {e}")
            else:
    ...
    try:
        ...
        if not VISUAL_BIBLE.get("approved", False):
            ...
            # [수정] 무인 구동을 위해 input 제거 및 기본값 '1' 설정
            if args.auto_approve:
                print(f"🤖 [Auto-Approve] 사용자 개입 없이 기본값 '1' (대립과 긴장)을 선택합니다.")
                choice = "1"
            else:
                print(f"🤖 [자동 설정] 사용자 개입 없이 기본값 '1' (대립과 긴장)을 선택합니다.")
                choice = "1" 
    ...
    if os.path.exists(profile_cache_path):
        ...
        try:
            with open(profile_cache_path, "r", encoding="utf-8") as f:
                p = json.load(f)
            print(f"  - 현재 저장된 주인공: {p.get('name')} ({p.get('age')}, {p.get('gender')})")
            print(f"  - 묘사: {p.get('visual_description', '')[:100]}...")
            
            # [추가] 대본 내용이 바뀌었는지 단순 체크 (첫 50자 비교)
            first_chars = full_text[:50].strip()
            if p.get('script_start') and p.get('script_start') != first_chars:
                print("\n" + "!"*80)
                print("⚠️ [경고] 대본 내용이 바뀐 것 같습니다! (기존 캐시는 다른 이야기일 수 있음)")
                print(f"  * 기존 시작: {p.get('script_start')[:30]}...")
                print(f"  * 현재 시작: {first_chars[:30]}...")
                print("!"*80 + "\n")
                if args.auto_approve:
                    char_choice = 'n' # 내용 바뀌었으면 자동 모드에서도 새로 분석
                else:
                    char_choice = input("👉 기존 주인공 사용(r) / '새 대본'으로 다시 분석(n): ").strip().lower()
            else:
                if args.auto_approve:
                    char_choice = 'r'
                else:
                    char_choice = input("👉 기존 주인공 사용(r) / 새로 파악(n): ").strip().lower()
        except Exception as e:
            print(f"⚠️ 캐릭터 프로필 로드 실패: {e}")
            char_choice = 'n'
...
if __name__ == "__main__":
    ...
    # 명령행 인자가 있다면 최우선 적용
    if args.script_path and args.script_path.endswith(".txt"):
