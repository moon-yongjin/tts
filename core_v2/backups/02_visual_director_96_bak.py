...
def split_script_by_duration(text, script_name="대본.txt", target_interval=6.0):
    ...
    # [강제 설정] 96장 고정 생성 (사용자 요청)
    target_image_count = 96
    # 한 장당 배분할 평균 글자 수
    chars_per_image = max(5, round(total_chars / target_image_count))
    
    print(f"⏱️  96장 고정 생성 모드: 총 {total_chars}자 -> {target_image_count}장 분할 (약 {chars_per_image}자당 1장)")
...
def get_consistent_prompts(script_chunk, prev_summary="None", current_location="Unknown", retries=2):
    ...
    - VARIETY (Style Guideline): {VISUAL_BIBLE.get('visual_variety_en', '')} (Note: Use this as a stylistic inspiration, do NOT repeat the same objects in every scene)
