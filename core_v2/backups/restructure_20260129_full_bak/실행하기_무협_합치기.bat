...
:: [최신 폴더 자동 찾기] Downloads 폴더 내 '무협_생성_'으로 시작하는 가장 최근 폴더를 찾습니다.
for /f "tokens=*" %%i in ('dir "%USERPROFILE%\Downloads\무협_생성_*" /b /ad /o-d 2^>nul') do (
    set "TARGET_DIR=%USERPROFILE%\Downloads\%%i"
    goto :found
)

:found
if "%TARGET_DIR%"=="" (
    echo ❌ '무협_생성_'으로 시작하는 폴더를 Downloads에서 찾을 수 없습니다.
    pause
    goto :EOF
)
