...
def direct_sound_design_fixed(script_text):
    """
    [High-Precision Rule] 70글자마다(공백 제외) 정확하게 효과음 태그를 삽입합니다.
    """
    print("📏 [High-Precision] 70자당 1개(정확도 100%) 태그 배치 중...")
    
    sound_map = {
        "문": "문소리", "돌문": "육중한돌문", "계단": "계단소리", "발걸음": "발소리", "발": "발소리",
        "비명": "날카로운비명", "소리": "효과음", "불빛": "횃불소리", "정적": "정적분위기",
        "바람": "바람소리", "빗줄기": "빗소리", "천둥": "천둥소리", "자물쇠": "자물쇠소리",
        "금괴": "금속마찰음", "궤짝": "궤짝여는소리", "기계": "기계작동음", "웃음": "웃음소리",
        "통곡": "울음소리", "애원": "애처로운분위기", "전율": "긴장감", "심장": "심장박동",
        "서찰": "종이바스락", "수첩": "책장넘기는소리", "칼": "칼날소리", "검": "검기소리",
        "탕약": "물마시는소리", "화분": "화분깨지는소리", "장롱": "장롱여는소리", "기침": "기침소리",
        "옷자락": "옷스치는소리", "찻잔": "도자기부딪히는소리", "차": "물따르는소리",
        "낙엽": "낙엽밟는소리", "숲": "숲속앰비언스", "까마귀": "까마귀소리", "늑대": "늑대울음소리",
        "종": "종소리", "겨울": "겨울바람소리", "눈": "눈밟는소리", "산": "산들바람",
        "부엉이": "부엉이울음소리", "밤": "귀뚜라미소리", "등불": "등불흔들리는소리"
    }
    
    # [백업용] 딱히 넣을 게 없을 때 돌아가며 나올 '병신 같지 않은' 분위기 효과음 목록
    atmospheric_fallbacks = [
        "겨울바람소리", "늑대울음소리", "먼종소리", "부엉이울음소리", 
        "으스스한분위기", "밤바람소리", "저벅저벅발소리"
    ]
    import random
        ...
        if re.match(r'\[SFX:.*?\]', part):
            ...
            for char in part:
                ...
                if char_acc >= target_interval:
                    # 현재 위치로부터 앞쪽 70자 내에서 키워드 검색
                    lookback = new_script[-70:]
                    found_sfx = None
                    for key, val in sound_map.items():
                        if key in lookback:
                            found_sfx = val
                            break
                    
                    # [수정] 아무것도 없으면 'None' 대신 분위기 있는 소리 자동 배정
                    if not found_sfx:
                        found_sfx = random.choice(atmospheric_fallbacks)
                    
                    new_script += f" [SFX:{found_sfx}] "
                    char_acc = 0
...
def process_script_sfx(script_text, output_dir="Library/sfx"):
    ...
    # [수정] 사용자 요청: 70자당 1개 미만이면 AI 감독 투입 (단, 대본이 이미 충분히 태깅되었다면 스킵)
    target_density = len(script_text) / 70
    if len(existing_tags) < target_density:
        print(f"📏 SFX 밀도 부족 (현재: {len(existing_tags)}개, 목표: {target_density:.1f}개) -> 70자 규칙 기반 자동 배치")
    ...
    try:
        ...
        for item in data:
            fname = str(item.get('filename', 'ambient_atmosphere')).replace(".mp3", "")
            if not fname or fname.lower() == "none" or fname.lower() == "null":
                fname = "ambient_atmosphere"
