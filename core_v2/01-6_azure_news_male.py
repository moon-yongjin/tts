...
# ==========================================================
# [사용자 설정 구역] - Step 01-6: 아주르 남성 뉴스 모드
# ==========================================================

VOICE_NAME = "ko-KR-InJoonNeural" # 봉진 대신 인준(InJoon)으로 변경 (뉴스톤에 적합)
PITCH = "+12.00%"                  # 피치 아주 높게 (+12)
SPEED = "1.3"                      # 읽는 속도 1.3
PROSODY_RATE = "+30.00%"           # 1.3배속은 +30% 
# Azure Speech 설정
...
def generate_segment(text, temp_audio_path, start_ticks_offset=0, start_idx=1):
    ...
    <speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="ko-KR">
        <voice name="{VOICE_NAME}">
            <prosody rate="{PROSODY_RATE}" pitch="{PITCH}">{clean_text}</prosody>
        </voice>
    </speak>
    '''
    ...
    def word_boundary_handler(evt):
        ...
        # word.audio_offset is in ticks (100ns)
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        ...
        print(f"⚠️ Azure 생성 실패: {result.reason}")
    return [], 0, current_idx

def run_news_production(script_text, output_path):
    # 3000자 단위로 분할
    chunks, segment = [], ""
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        for i, chunk in enumerate(chunks):
            print(f"🎙️ [Part {i+1}/{len(chunks)}] Azure News 생성 중 ({VOICE_NAME})...")
    ...
    print(f"✅ 최종 AI 뉴스 파일 생성 완료: {os.path.basename(output_path)}")

if __name__ == "__main__":
    target_file = sys.argv[1] if len(sys.argv) > 1 else os.path.join(PROJ_ROOT, "대본.txt")
    print(f"🚀 [Step 01-6] Azure News Male (Pitch:{PITCH}, Speed:{SPEED}) 시작")
    ...
    if os.path.exists(target_file):
        ...
        if script_text:
            ...
            output_name = f"Azure_News_Male_P12_S1.3_{timestamp}.mp3"
            output_path = os.path.join(DOWNLOADS_DIR, output_name)
            run_news_production(script_text, output_path)
            print(f"✨ 작업 완료! 결과물 위치: {DOWNLOADS_DIR}")
