...
# 3단계 방어: 극단적 언어 제약
INSTRUCT = """당신은 서울 표준어를 사용하는 남성 성우입니다. 
...
class QwenMaleVoicesGenerator:
    ...
    def generate_for_speaker(self, speaker, script_text, base_filename, speed=1.2, pitch=0.5):
        ...
        print(f"\n🎙️  남성 성우 생성 시작: {speaker} (Speed: {speed}, Pitch: {pitch})")
    ...
    def run_all(self, script_text, target_file, speed=1.2, pitch=0.5):
        base_filename = os.path.splitext(os.path.basename(target_file))[0]
        for speaker in MALE_SPEAKERS:
            self.generate_for_speaker(speaker, script_text, base_filename, speed, pitch)
...
if __name__ == "__main__":
    ...
    pitch = float(sys.argv[3]) if len(sys.argv) > 3 else 0.5
    
    if os.path.exists(target_file):
        with open(target_file, "r", encoding="utf-8") as f:
            script_text = f.read().strip()
        
        generator = QwenMaleVoicesGenerator()
        generator.run_all(script_text, target_file, speed, pitch)
    else:
