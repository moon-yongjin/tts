...
SPEED_FACTOR = 1.1
PITCH_SHIFT = 0.0  # 커스텀 목소리는 피치 조정 불필요
...
# [패치 완료] MLX-Audio 및 차원 교정 패치만 유지합니다.
print("🎲 시드 제어 준비 완료 (Global Seed Strategy)")

class QwenCustomVoiceGenerator:
    def __init__(self):
        ...
        # 커스텀 보이스 체크
        if not os.path.exists(CUSTOM_VOICE_WAV):
            print(f"❌ 커스텀 보이스 파일을 찾을 수 없습니다: {CUSTOM_VOICE_WAV}")
            sys.exit(1)
        print(f"✅ 커스텀 보이스 연결 완료: {os.path.basename(CUSTOM_VOICE_WAV)}")
    ...
    def split_chunks(self, text, max_chars=45):
        """텍스트를 자연스러운 청크로 분할 (문장 부호 + 길이 기반, 01-3 방식)"""
    ...
    def generate_chunk_with_retry(self, chunk, chunk_idx):
        # [시드 고정] 8번 황금 목소리의 행운을 재현
        mx.random.seed(8)
        
        for attempt in range(MAX_RETRIES):
            try:
                # 커스텀 목소리 모드: ref_audio와 ref_text 사용
                results = self.model.generate(
                    text=chunk, 
                    ref_audio=CUSTOM_VOICE_WAV, 
                    ref_text=CUSTOM_VOICE_REF_TEXT,  # 정확한 참조 텍스트 (핵심!)
                    language="Korean",
                    instruct=INSTRUCT,
                    **GEN_KWARGS
                )
    ...
    def run(self, script_text, output_path):
        ...
        print(f"📦 Total Custom Voice Chunks: {len(chunks)} (최대 50자)")
        ...
        for i, chunk in enumerate(chunks):
            ...
            if i < len(chunks) - 1:
                pause_ms = 400 if any(chunk.endswith(p) for p in ['.', '?', '!']) else 200
...
if __name__ == "__main__":
    ...
    if os.path.exists(target_file):
        ...
        if script_text:
            ...
            output_name = f"{base_filename}_CustomVoice_G8_{timestamp}.mp3"
            output_path = os.path.join(DOWNLOADS_DIR, output_name)
            
            generator = QwenCustomVoiceGenerator()
