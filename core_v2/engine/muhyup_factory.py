...
# [경로 설정]
ENGINE_DIR = os.path.dirname(os.path.abspath(__file__)) # .../core_v2/engine
CORE_DIR = os.path.dirname(ENGINE_DIR) # .../core_v2
ROOT_DIR = os.path.dirname(CORE_DIR) # .../tts (TRUE ROOT)
...
if os.path.exists(CONFIG_PATH):
    try:
        with open(CONFIG_PATH, "r", encoding="utf-8") as f:
            config = json.load(f)
            if config.get("Azure_Speech_Key"): SPEECH_KEY = config.get("Azure_Speech_Key")
            if config.get("Azure_Region"): SPEECH_REGION = config.get("Azure_Region")
            print(f"✅ Config 로드 완료: {SPEECH_REGION}")
    except: pass
...
def clean_for_output(text):
    # [설정] Azure 전용: SFX는 지우지 않고 generate_voice의 bookmark 처리에서 다룸
...
def generate_voice(text, output_path, start_index=1):
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        ...
        print(f"❌ Azure TTS 취소/실패: {cancellation_details.reason}")
        print(f"   세부 오류: {cancellation_details.error_details}")
...
if __name__ == "__main__":
    ...
    script_candidates = [
        ...
        os.path.join(CORE_DIR, target_file),
    ...
    if script_path:
        ...
        if script_text:
            ...
            for i, chunk in enumerate(chunks):
                ...
                full_path = os.path.join(os.path.expanduser("~"), 'Downloads', part_name)
            ...
            print(f"\n✨ 모든 작업이 완료되었습니다! 결과물 위치: {os.path.join(os.path.expanduser('~'), 'Downloads')}")