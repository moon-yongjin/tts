...
STYLES = {
    ...
    "Flux Realism": "Photorealistic, 8k RAW photo, Fujifilm XT4, Cinematic Lighting, Skin texture detail, Weathered face, Hyper-realistic eyes, Subtle cinematic grain, <lora:flux-RealismLora:0.8>",
}
SELECTED_STYLE = "Flux Realism" 
VISUAL_BIBLE = {}
...
def get_consistent_prompts(script_chunk, prev_summary="None", current_location="Unknown", retries=2):
    ...
    IMPORTANT: 
    - DO NOT use words like: "Beautiful", "Masterpiece", "Best quality", "Artistic", "Oil painting", "Digital art".
    - Focus on: "Raw", "Photorealistic", "Texture", "Imperfect details", "Cinematic".
...
def generate_single_image(scene, global_num):
    """Draw Things App API 호출 (Flux + LoRA 최적화)"""
    prompt = scene['visual_prompt']
    filename = f"{global_num:03d}_{SELECTED_STYLE}.png"
    filepath = os.path.join(DOWNLOAD_DIR, filename)
    
    # [변경] 파일명 중복 시 카운트 증가 (덮어쓰기/스킵 방지)
    base_name = f"{global_num:03d}_{SELECTED_STYLE}"
    filename = f"{base_name}.png"
    filepath = os.path.join(DOWNLOAD_DIR, filename)
    
    counter = 1
    while os.path.exists(filepath):
        filename = f"{base_name}_v{counter}.png"
        filepath = os.path.join(DOWNLOAD_DIR, filename)
        counter += 1
    ...
    # [변경] Flux Realism에 맞춘 프롬프트 조립
    full_prompt = (
        f"{style_text}, film grain, high ISO, natural imperfections, skin pores, raw photo, "
        f"Artistic scene: {prompt}"
    )
    
    # [변경] Flux [dev/schnell] Realism Tuning 적용
    payload = {
        "prompt": full_prompt,
        "negative_prompt": "cartoon, 3d, painting, drawing, sketch, blurry, plastic, smooth, doll, low resolution, bad anatomy, text, watermark",
        "steps": 15,         # [변경] 25 -> 15 (Speed/Quality Balance)
        "width": 720,       
        "height": 1280,       
        "cfg_scale": 2.5,    # [변경] 1.0 -> 2.5 (Lower guidance for natural look)
