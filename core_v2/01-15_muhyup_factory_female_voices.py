...
FEMALE_SPEAKERS = ["serena", "vivian", "sohee", "ono_anna"]

# 페르소나별 명령문
NORMAL_INSTRUCT = "당신은 서울 표준어를 사용하는 여성 성우입니다. 정확하고 단호한 한국어 어조로 낭독하세요."
GRANDMA_INSTRUCT = """70대 이상의 나이가 지긋한 할머니 목소리로 연기하세요. 
...
class QwenFemaleVoicesGenerator:
    def __init__(self):
    ...
    def split_chunks(self, text, max_chars=45):
        ...
        for i in range(0, len(sentences), 2):
            ...
            if len(full_sentence) > max_chars:
                ...
                if len(current_chunk) + len(full_sentence) <= max_chars: current_chunk += full_sentence
                else:
    ...
    def generate_sample(self, speaker, mode, script_text, base_filename):
        # 모드별 설정
        is_grandma = (mode == "Grandma")
        speed = 1.2
        pitch = -1.5 if is_grandma else 0.0
        instruct = GRANDMA_INSTRUCT if is_grandma else NORMAL_INSTRUCT
        
        timestamp = datetime.datetime.now().strftime("%m%d_%H%M")
        output_name = f"{base_filename}_{speaker}_{mode}_S{speed}_P{pitch}_{timestamp}.mp3"
        output_path = os.path.join(DOWNLOADS_DIR, output_name)
        
        chunks = self.split_chunks(self.clean_text(script_text))
        print(f"\n🎙️  여성 성우 [{speaker}] 모드 [{mode}] 시작")
    ...
    def run_all(self, script_text, target_file):
        ...
        # 사용자의 요청에 따라 Grandma 모드만 집중 생성 (따옴표 제거 버전)
        for mode in ["Grandma"]:
            for speaker in FEMALE_SPEAKERS:
                self.generate_sample(speaker, mode, script_text, base_filename)
...
if __name__ == "__main__":
    ...
    if os.path.exists(target_file):
        ...
        generator = QwenFemaleVoicesGenerator()
