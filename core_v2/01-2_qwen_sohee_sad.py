...
# 1. 내레이션 (따옴표 밖) - 인자한 할머니
NARRATION_PITCH = 0.0
NARRATION_SPEED = 1.1
NARRATION_INSTRUCT = "정확한 서울 표준어를 사용하는 70대 할머니 성우입니다. 중국어나 외국어 억양을 절대 섞지 말고, 세월의 무게가 느껴지는 깊은 울림이 있는 순수 한국어 어조로 차분하게 낭독하세요."

# 2. 대사 (따옴표 안) - 날카로운 딸
DIALOGUE_PITCH = 1.5
DIALOGUE_SPEED = 1.0
DIALOGUE_INSTRUCT = "서울 표준어를 사용하는 20대 여성 성우입니다. 중국어 억양을 절대 배제하고, 짜증이나 서운함이 섞인 단호하고 날카로운 한국어 어조를 유지하세요."
...
class production_sohee_generator:
    ...
    def parse_script(self, text):
        """따옴표 기준 세그먼트 분리 후, 각 세그먼트를 다시 청크로 분리"""
        segments = []
    ...
    def run(self, script_path):
        ...
        print(f"📦 총 {len(segments)}개의 파트로 정밀 분석 완료")

        timestamp = datetime.datetime.now().strftime("%m%d_%H%M")
        base_name = f"소희피치_슬픔_{timestamp}"
        ...
        print(f"\n✨ 생성 완료!")
        print(f"📂 결과 MP3: {mp3_path}")
        print(f"📂 결과 SRT: {srt_path}")

if __name__ == "__main__":
    script = sys.argv[1] if len(sys.argv) > 1 else "/Users/a12/projects/tts/대본.txt"
    gen = production_sohee_generator()
    gen.run(script)
