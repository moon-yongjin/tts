...
# FFMPEG_DIR / FFMPEG_EXE 설정 제거 -> 시스템 PATH 사용 (Mac 환경 호환)
...
def create_background_audio():
    ...
    # [변경] 통합된(Merged) 파일 처리로 변경 (Part 파일 삭제됨)
    now = time.time()
    srt_files = []
    for f in os.listdir(DOWNLOADS_DIR):
        # '_Full_Merged.srt' 또는 그냥 '.srt' (최신)
        if f.lower().endswith(".srt") and ("_Full_Merged" in f):
            full_path = os.path.join(DOWNLOADS_DIR, f)
            if (now - os.path.getmtime(full_path)) < 3600: # 1시간 이내
                srt_files.append(f)
    
    # 만약 Merged 파일이 없으면, 일반 srt 중 가장 최신 것 1개라도 잡기 (Fallback)
    if not srt_files:
         candidates = [f for f in os.listdir(DOWNLOADS_DIR) if f.lower().endswith(".srt") and "_part" not in f]
         if candidates:
             latest = max([os.path.join(DOWNLOADS_DIR, f) for f in candidates], key=os.path.getmtime)
             srt_files.append(os.path.basename(latest))

    if not srt_files:
        print("❌ 처리할 자막 파일(_Full_Merged.srt)을 찾을 수 없습니다.")
        return

    print(f"📂 발견된 파일: {len(srt_files)}개")

    # [사용자 요청] 카운터 002부터 시작 (기존 유지)
    current_counter = 2
