...
def get_stt_model():
    ...
    if _stt_model is None:
        print(f"📡 Whisper STT 모델 로딩 중...")
        _stt_model = whisper.load_model(STT_MODEL_NAME)
    return _stt_model
...
def generate_voice_qwen(text, output_path):
    ...
    print(f"✍️ 자막 생성 중 (Whisper)...")
    stt = get_stt_model()
    result = stt.transcribe(temp_wav, language="ko")
    ...
    os.rename(temp_wav, final_output)
    print(f"✅ 완료: {os.path.basename(final_output)}")
    return True
