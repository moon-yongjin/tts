...
def get_consistent_prompts(script_chunk, prev_summary="None", current_location="Unknown", retries=2):
    ...
    1. **LOCATION DETECTION**: 
       - If the script mentions a specific place (e.g., 'deep in the bamboo forest', 'inside the dark cave', 'market square'), you MUST update the location. 
       - Describe the NEW environment in extreme detail (architecture, lighting, weather, textures).
       - If no location change is implied, MAINTAIN the previous location with consistent details.
       
    2. **ATMOSPHERIC DETAIL**: Describe light sources (torches, moonlight, filtered through leaves), atmospheric effects (mist, dust, rain), and specific objects that ground the scene.
    
    3. **CINEMATIC COMPOSITION**:
       - Use "Rule of Thirds" and "Wide Angle" shots often to show the world, not just the character.
       - Place the character asymmetrically. 
       - If the character is mentioned doing an action, show the character + environment.
       
    4. **STRICT NO-TEXT RULE**: NEVER include any letters, watermarks, or text in the visual prompt.
    ...
    {{
      ...
      "current_location": "The specific name and 1-line description of the environment in this scene"
    }}
    """
    ...
    for attempt in range(retries + 1):
        try:
            ...
            next_summary = data.get('summary_for_next', prev_summary)
            next_location = data.get('current_location', current_location)
            
            return scenes, next_summary, next_location
