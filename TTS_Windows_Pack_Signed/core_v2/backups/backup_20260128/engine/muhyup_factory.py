...
def apply_srt_adjustment(srt_path, original_script_part):
    """Fuzzy Matching을 사용하여 SRT 자막을 원본 대본과 완벽하게 동기화"""
    ...
    # 1. 원본 대본 단어화
    clean_text = re.sub(r'\[SFX:.*?\]', ' ', original_script_part)
    clean_text = re.sub(r'\[(대사|묘사|지문|설명|SFX|챕터|CHAPTER).*?\]', ' ', clean_text, flags=re.IGNORECASE)
    ...
    for entry in srt_entries:
        ...
        if indices:
            ...
            if start <= end:
                entry_text = " ".join(script_words[start:end+1])
                last_script_idx = end
...
def generate_voice(text, output_path, start_index=1):
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        ...
        srt_file_path = output_path.replace(".mp3", ".srt")
        with open(srt_file_path, "w", encoding="utf-8-sig") as f: f.writelines(srt_entries)
        
        # [자동 자막 보정] 대본 원본과 대조하여 오타/특수문자 복원
        try:
            apply_srt_adjustment(srt_file_path, text)
        except Exception as e:
            print(f"⚠️ 자막 보정 중 알림: {e}")
