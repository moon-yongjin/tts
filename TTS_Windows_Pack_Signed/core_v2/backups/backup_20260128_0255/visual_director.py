...
def get_consistent_prompts(script_chunk, prev_summary="None", retries=2):
    """Gemini를 사용해 일관성 있는 이미지 프롬프트 추출 (재시도 로직 추가)"""
    style_desc = STYLES.get(SELECTED_STYLE, STYLES["애니메이션"])
    
    # [추가] 흑백 스타일일 경우 컬러 사용을 강력하게 금지하는 명령어 추가
    monochrome_rule = ""
    if SELECTED_STYLE in ["스케치", "수묵화"]:
        monochrome_rule = "CRITICAL: This is a BLACK AND WHITE style. Do NOT use any color descriptors. Use only shades of gray, black, and white. NO COLOR allowed."
    
    prompt = f"""
    [Task] Create 1 detailed visual prompt for the provided script part.
    [Visual Style Preset] {SELECTED_STYLE}: {style_desc}
    {monochrome_rule}
    
    [STRICT VISUAL BIBLE - STYLE & CHARACTER]
    - CHARACTER: {VISUAL_BIBLE.get('character_en', VISUAL_BIBLE.get('character', ''))}
    - BASE SETTING: {VISUAL_BIBLE.get('background_en', VISUAL_BIBLE.get('background', ''))}
    - STYLE GUIDELINE: {VISUAL_BIBLE.get('art_style_override_en', VISUAL_BIBLE.get('art_style_override', ''))}
    - VISUAL VARIETY: {VISUAL_BIBLE.get('visual_variety_en', '')}
    
    [Previous Story Summary] {prev_summary}
    [Current Script Part]
    {script_chunk}
    
    [CORE RULES] 
    1. PROMPT: Combine the Visual Bible data with the specific actions and LOCATION mentioned in the current script.
    2. COMPOSITION: Avoid placing characters in the center. Use the **Rule of Thirds** and **Asymmetric Framing** (place characters on the left or right side of the frame).
    3. SOLO SHOTS: If a character is alone, zoom out slightly or use a wider angle to show life-like surrounding objects and the environment clearly.
    4. BACKGROUND: If the script indicates a location change (e.g., basement, secret passage, stone wall, cave, outside), you MUST REPLACE the Base Setting with this new location. Do NOT include elements of the old room in the new location.
    5. CONSISTENCY: Keep the character's appearance strictly consistent as described in the Bible.
    6. ART STYLE: Always maintain the {SELECTED_STYLE} and Art Style Guideline.
    7. FOCUS: 16:9 cinematic composition, high quality.
    8. **NO TEXT**: Strict rule - NO letters, NO words, NO characters, NO watermarks allowed in the image.
    
    [Output Format] JSON with 'scenes' (array with 1 object containing 'narration' and 'visual_prompt') and 'summary_for_next'.
    """
    
    for attempt in range(retries + 1):
        try:
            print(f"🤖 Gemini 분석 중... (시도 {attempt+1}/{retries+1})")
            response = client.models.generate_content(
                model="publishers/google/models/gemini-2.0-flash-001", 
                contents=prompt,
                config=types.GenerateContentConfig(
            ...
            return data.get('scenes', []), data.get('summary_for_next', prev_summary)
        except Exception as e:
            print(f"⚠️ Gemini 시도 {attempt+1} 에러: {e}")
            if attempt < retries:
                time.sleep(2)
            else:
                print(f"❌ Gemini 분석 최종 실패")
                return [], prev_summary
...
def run_pipeline(full_text, script_name="대본.txt"):
    ...
    print(f"📦 총 {total_images}개의 파트 분석 및 생성을 시작합니다.")
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        for i, chunk in enumerate(chunks):
            print(f"\n--- [파트 {i+1}/{len(chunks)}] Gemini 분석 중 ---")
            
            # 1. 일관성 있는 프롬프트 추출 (Gemini)
            scenes, current_summary = get_consistent_prompts(chunk, current_summary)
